...
public static class Utils
{
    ...
    public static async Task WriteToDiskAsync(IWebHostEnvironment environment, string fileName, IFormFile file)
    {
        string filePath = Path.Combine(GetImageFolderPath(environment), $"{fileName}.jpg");
        Directory.CreateDirectory(GetImageFolderPath(environment));
    ...
    public static async Task WriteToTempAsync(IWebHostEnvironment environment, string fileName, IFormFile file)
    {
        ...
        Directory.CreateDirectory(GetTempFolderPath(environment));
        using var stream = File.OpenWrite(filePath);
        await file.CopyToAsync(stream);
    }
